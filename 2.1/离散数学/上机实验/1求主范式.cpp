...
/*基本思路：
  1.(core)先将合式读入，将其转化为逆波兰式储存至新的数组内，
    并开数组记录变量是否被访问过
  2.(core)将指针指向最开头第一个变量，从此开始dfs遍历每一种可能，
    记录极大项和极小项出现的次数和赋值的内容（利用vector数组）
  3.输出结果
*/
...
void dfs(int cur,int cur_var){//cur指向当前位置，cur_var记录已被赋值的变量个数
	if(RPN[cur]=='#'){//到了底了
		...
		if(final_result==0){//成假，为一极大项
		    int temp=0;
		    for(int time=0,point=cur_var-1;point>=0;point--,time++){//计算对应的项数
		    	temp+=(var_value[point]*(int)pow(2,time));
			}
...
int main(){
	...
	bigone.lenth=0;
    smallone.lenth=0;
	memset(smallone.array,0,sizeof(smallone.array));//将极小项数组重置
	memset(bigone.array,0,sizeof(bigone.array));//将极大项数组重置
